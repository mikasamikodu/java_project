        ...
        $("#editBtn").click(function(){
        	...
        	if($.trim(name.val()) == ""){
       		 //icon5效果是笑脸，shift:6效果是抖动，time:1000效果持续时间是1秒
        		 layer.msg("许可名称不能为空", {time:1000, icon:5, shift:2}, function(){
        			 name.val("");
        			 name.focus();
        		 });
        		 return false;
        	...
        	if($.trim(url.val()) == ""){
       		 //icon5效果是笑脸，shift:6效果是抖动，time:1000效果持续时间是1秒
        		 layer.msg("许可URL不能为空", {time:1000, icon:5, shift:3}, function(){
        			 url.val("");
        			 url.focus();
        		 });
        		 return false;
        	...
        	$.ajax({
        		...
        		beforeSend : function(){
        			layerIndex = layer.msg("正在修改许可信息,请稍候...",{icon : 6});
        			return true;
        		},
        		success : function(result){
        			layer.close(layerIndex);
        			layer.msg("修改许可信息成功", {time:1000, icon: 6});
        			if(result.success){
        		...
        		error : function(){
        			layer.msg("修改许可信息出现问题", {time:1000, icon: 6});
        		}
        	});
        });
    </script>
